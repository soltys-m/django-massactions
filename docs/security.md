...
* **Selection cookie.** The value is produced by `massactions:encrypt` and signed with `SECRET_KEY`
  (`django.core.signing`, salt `massactions.selection`, zlib compressed, valid for
  `MASSACTIONS_SELECTION_MAX_AGE` seconds). A tampered, forged or expired value is treated as "nothing
  selected". The payload is readable (base64 of compressed JSON) but contains only ids and flags, and every
  id still has to pass `get_queryset()` and `restrict_queryset()`. Malformed cookies, invalid ids and stale
  ids are handled without errors.
