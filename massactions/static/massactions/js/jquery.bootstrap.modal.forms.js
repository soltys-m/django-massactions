...
Modified for django-massactions:
  - `showModal(settings)` global: open a modal form programmatically (used after the selection cookie is stored)
  - `initOnClick: false` option: skip binding the click handler in `.modalForm()`
  - `successCallback` / `callback` hooks in async mode
*/
...
(function ($) {
    ...
    var modalForm = function (settings) {
        $(settings.modalID).find(settings.modalContent).load(settings.formURL, function () {
            ...
            $(settings.modalID).modal("show").on('shown.bs.modal', function(e) {
            ...
            if (asyncSettingsValid) {                
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        if (asyncSettings.successCallback) {
                            asyncSettings.successCallback(formdata, asyncSettings.successCallbackParam);
